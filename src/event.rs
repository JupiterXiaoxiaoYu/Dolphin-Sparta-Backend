...
use crate::{
    config::get_modifier,
    state::{Modifier, Object, HelloWorldPlayer, Owner},
};
...
pub fn apply_modifier(player: &mut HelloWorldPlayer, object: &mut Object, modifier: Modifier) -> bool {
...
fn apply_object_modifier(
    ...
    let mut player = HelloWorldPlayer::get(owner_id).unwrap();
    ...
    pub fn tick(&mut self) {
        ...
        while let Some(head) = self.list.front_mut() {
            if head.delta == 0 {
                ...
                let obj_id = HelloWorldPlayer::generate_obj_id(&owner_id, objindex);
