...
function createCommand(command, nonce) {
...
export class Player {
    ...
    async installPlayer() {
        ...
        try {
            let finished = await this.rpc.sendTransaction(new BigUint64Array([createCommand(CMD_INSTALL_PLAYER, 0n), accountInfo[1], accountInfo[2], 0n]), this.processingKey);
    ...
    //     }
    //   }
    async sendGameCommand(command, nonce, param = 0) {
        let accountInfo = new LeHexBN(query(this.processingKey).pkx).toU64Array();
        try {
